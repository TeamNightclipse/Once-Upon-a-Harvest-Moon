...
class ScalaNetworkWrapper(channelName: String) {
  ...
  def init:                                   IdState[Int] = IdState.init
  def registerMessages(state: IdState[Unit]): Unit         = state.run(0)

  def registerMessage[A: ClassTag: MessageConverter](implicit handler: MessageHandler[A, _]): IdState[Unit] =
    IdState(i => (i + 1, registerMessage(handler, i.toByte, handler.side)))

  def registerMessage[A: ClassTag, Reply](handler: MessageHandler[A, Reply], discriminator: Byte, side: Side)(implicit converter: MessageConverter[A]): Unit = {
