...
object ScalaIndexedMessageChannel {
  val InboundPacketTracker: AttributeKey[ThreadLocal[WeakReference[FMLProxyPacket]]] = AttributeKey.valueOf(s"${LibMod.Id}:inboundpacket")
}
...
class ScalaIndexedMessageChannel extends ChannelDuplexHandler {
  ...
  private val encoder = new MessageToMessageEncoder[AnyRef]() {
    ...
    override def encode(ctx: ChannelHandlerContext, msg: AnyRef, out: util.List[AnyRef]): Unit = {
      ...
      discriminatorToConverter(discriminator).asInstanceOf[MessageConverter[Any]].toBytes(msg, buf)
  ...
  private val decoder = new MessageToMessageDecoder[AnyRef]() {
    ...
    override def decode(ctx: ChannelHandlerContext, msg: AnyRef, out: util.List[AnyRef]): Unit = {
      ...
      if (payload.readableBytes < 1) LogHelper.error(s"The ScalaIndexedMessageChannel has received an empty buffer on channel ${ctx.channel
      ...
      discriminatorToConverter.get(discriminator) match {
        case Some(converter) =>
          ctx.attr(ScalaIndexedMessageChannel.InboundPacketTracker).get.set(new WeakReference[FMLProxyPacket](fmlProxyPacket))
          val newMsg = converter.fromBytes(payload.slice())
          out.add(newMsg.asInstanceOf[AnyRef])
        case None => LogHelper.error(s"Undefined message for discriminator $discriminator in channel ${fmlProxyPacket.channel}")
      }
    }
  }
